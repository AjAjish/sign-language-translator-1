    ...
    <aside style={{
      ...
      <div style={{ padding: 'var(--space-4) var(--space-3)', borderBottom: '1px solid var(--border)' }}>
        <button onClick={onBack} style={{
          ...
          marginBottom: 'var(--space-3)', transition: 'color 0.2s', padding: 'var(--space-2)', borderRadius: 'var(--radius-lg)',
            ...
            {MODES.filter(m => m.group === group).map(m => {
              ...
              return (
                <button key={m.id} onClick={() => setMode(m.id)} style={{
                  ...
                  textAlign: 'left', transition: 'all 0.2s', minHeight: '44px'
                }}
      ...
      <div style={{ padding: 'var(--space-3) var(--space-2)', borderTop: '1px solid var(--border)' }}>
        <div style={{
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: backendStatus === 'online' ? 'var(--success)' : 'var(--text-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              Backend {backendStatus === 'online' ? 'Online' : 'Offline'}
            </div>
            <div style={{ fontSize: '0.65rem', color: 'var(--text-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>localhost:8000</div>
          </div>
        </div>
      </div>
    </aside>
